  ...
  ([paragraphs already-split-paragraphs]
   (if (empty? paragraphs)
     ...
     (let [lines (remove #(= "*" %) (clojure.string/split-lines paragraphs))
           ...
           lines (remove #(re-matches #"que                  TACKON.*" %) lines)
           ...
           lines (remove #(re-matches #"- together; completely.*" %) lines)
           ...
           lines (remove #(re-matches #"sex                  PREFIX.*" %) lines)
           lines (remove #(re-matches #"it                   SUFFIX.*" %) lines)
           lines (remove #(re-matches #"ne.*PREFIX.*" %) lines)
           lines (remove #(re-matches #"semi                 PREFIX.*" %) lines)
           lines (remove #(re-matches #"semi-, half, partly.*" %) lines)
